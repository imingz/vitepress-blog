...
源代码来自：[yidocs-auto](https://github.com/chenbimo/yicode/blob/master/helpers/yidocs-auto/index.js)

不太适合我的项目结构，就拿来改了改。

主要是改了根目录，现在可以把根目录当参数传进去了。
...
let { sideBar, navBar } = generateAutoBar("docs"); // [!code hl]
...
1. 所有文件和目录，都要以 `[0-9]+-` 开头
2. 文件使用 `分类-[项目]-目录-文件` 的方式来进行组织。(项目层级可选（未测试）)

## 待办

1. 增加支持如果存在 `0` 开头文件，则使用 `0` 开头文件为默认文件。（约定 `0` 开头文件为索引文件，链接到侧边栏文件夹 `link` 和导航栏 `link` 上）
